...
async function init() {
	...
	// EXPLAIN: so we are loading fox moddel; not that is important
	// for this lesson, we can remember from last lesson that we can
	// still load default glTF model format even we have draco enabled
	// meaning it will be decided under the hood that we are not using
	// draco and that we are loading default format without needing to
	// disable draco
	const foxModel = await gltfLoader.loadAsync(
		'/models/Fox/glTF/Fox.gltf',
	);

	// console.log(foxModel.scene);
	// EXPLAIN: model is huge, so we need to scale it down
	// it is not good idea to mess with the scale of the children
	// so what we can do here is scale the loaded scene and add it
	// directly to our scene
	foxModel.scene.scale.setScalar(0.025);

	// EXPLAIN: when you print you will see that you have
	// three AnimationClip instances inside animations array
	console.log(foxModel.animations);

	// EXPLAIN: creating animation mixer
	const mixer = new THREE.AnimationMixer(foxModel.scene);
	// EXPLAIN: adding one of AnimationClip instances to the mixer;
	// we get AnimationAction instance
	const action1 = mixer.clipAction(foxModel.animations[0]);
	const action2 = mixer.clipAction(foxModel.animations[1]);
	const action3 = mixer.clipAction(foxModel.animations[2]);

	// EXPLAIN: it won't play if just do this
	action2.play();
	// EXPLAIN: you must tell AnimationMixer to upadate on each frame
	// what we did bellow in animation loop
	...
	function tick(timestamp: number) {
		...
		// EXPLAIN: updating animationMixer with delta time
