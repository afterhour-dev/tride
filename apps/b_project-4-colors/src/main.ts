...
const debugObject = {
	...
	randomness: 0.2,
	randomnessPow: 3,
	// EXPLAIN: one color is used in the center warmer one
	// and other one will be used more outside and it will be colder
	insideColor: '#a54841',
	outsideColor: '#3b5284',
};
...
/* async  */ function init() {
	...
	const generateGalaxy = () => {
		...
		// EXPLAIN: we must enable vertexColors
		material.vertexColors = true;

		// EXPLAIN: create attribute
		const colors = new Float32Array(debugObject.count * 3);

		// EXPLAIN: creating instances of Color
		...
		for (let i = 0; i < debugObject.count * 3; i++) {
			...
			// EXPLAIN: mixing colors
			// EXPLAIN: why we used radiuses is it because values are bellow 1 like this?
			// 0.5 would be like balanced interpolation but we don't want that
			const mixedColors = colorInside.clone();
			mixedColors.lerp(colorOutside, radius / debugObject.radius);
			...
			// EXPLAIN: deling with vertex colors (red green blue)
			// colors[i3] = 1;
			// colors[i3 + 1] = 0;
			// colors[i3 + 2] = 0;

			// EXPLAIN: next three lines
		...
		// EXPLAIN: setting colors attribute
		geometry.setAttribute(
			'color',
	...
	galaxyTweaks
		...
		.step(0.001)
		.onFinishChange(generateGalaxy);
	// EXPLAIN: gui for colors
	galaxyTweaks
		.addColor(debugObject, 'insideColor')
		.onFinishChange(generateGalaxy);
	galaxyTweaks
		.addColor(debugObject, 'outsideColor')
		.onFinishChange(generateGalaxy);
