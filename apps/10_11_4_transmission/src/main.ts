...
async function init() {
	...
	*/
	...
	// from map to alphaMap; or better comment them in and out
	// to see the effect (and if we remove the textures we will
	// have nice translucent material just like some glass)
	...
	*/
	...
	awsomeTweaks
		...
		.name(
			'MeshPhysicalMaterial transmission effect realted tweaks (current lesson).\nIgnore ones above, they are mostly disabled except metalness\nand roughness',
		)
		.disable();
