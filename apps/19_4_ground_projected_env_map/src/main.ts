...
// EXPLAIN: we need GroundProjectedSkybox
...
const hdrMapLoadingManager = new THREE.LoadingManager();

const hdrLoader = new HDRLoader(hdrMapLoadingManager);

hdrMapLoadingManager.onProgress = (filePaths: string) => {
	console.log('progess ', filePaths);
};
hdrMapLoadingManager.onLoad = () => {
	console.log('HDR loaded');
};
hdrMapLoadingManager.onError = (e) => {
	console.error(e);
};
hdrMapLoadingManager.onStart = (filePath) => {
...
async function init() {
	...
	// EXPLAIN: we are only setting environment, not background
	...
	// EXPLAIN: setting environmentIntesity to 1
	scene.environmentIntensity = 1.0; // also default is 1.0

	// EXPLAIN: I'll also leave these ones to default values
	// scene.backgroundBlurriness = 0.2; // default is 0.0
	// scene.backgroundBlurriness = 0;
	// scene.backgroundIntensity = 2.4; // default is 1.0
	// scene.backgroundIntensity = 1;

	// EXPLAIN: now we add GroundedSkybox with our environment
	// map and other settings

	const skyboxHeight = 15;
	// EXPLAIN: also make sure to use this value for the
	// far avlaue of the camera but make far bigger than this
	...
	// EXPLAIN: why position y needs to be the same as height here
	...
	gamingConsoleModel.scene.position.x = -3.5;
	...
	const camera = new THREE.PerspectiveCamera(
		...
		0.1,
		// EXPLAIN: making far value to be related to skyboxRadius
		skyboxRadius * 3,
		// 100,
	);
	...
	// EXPLAIN: so this will force that we don't get out of the orb
	// as ve zoom
