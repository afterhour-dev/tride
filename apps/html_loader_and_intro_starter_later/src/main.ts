...
// EXPLAIN: using WebGPU instead of WebGL anyway even it has a bug I mentioned in readme
...
import { getRequiredElement, modelSetup } from './util';

// for loading textures -----------------------------------------

// const hdrLoadingManager = new THREE.LoadingManager();
const envMapTextureLoadingManager = new THREE.LoadingManager();
// const hdriLoader = new HDRLoader(envMapTextureLoadingManager);
const cubeTextureLoader = new THREE.CubeTextureLoader(
	envMapTextureLoadingManager,
);

envMapTextureLoadingManager.onProgress = (filePaths: string) => {
	console.log('progess ', filePaths);
};
envMapTextureLoadingManager.onLoad = () => {
	console.log('environment map cube texture loaded');
};
envMapTextureLoadingManager.onError = (e) => {
	console.error(e);
};
envMapTextureLoadingManager.onStart = (filePath) => {
...
async function init() {
	...
	// EXPLAIN: error happening when I set equirectangular
	// environmentMapAbandonedGarageTexture.mapping =
	// 	THREE.EquirectangularReflectionMapping;
	// EXPLAIN: but setting this mapping works (assuming that equirectangular is only for .hdr or .exr
	// and not for cube map and I nade a mistake by setting it)
	environmentMapAbandonedGarageTexture.mapping =
	...
	// EXPLAIN: PCFSoftShadowMap is deprecated so we are using PCFShadowMap
	// we also need to remove soft option in gui
 ...
 */
	...
	// EXPLAIN: shadow.radius doesn't work with PCFSoftShadowMap,
	// but also PCFSoftShadowMap is deprecated
	directionalLight.shadow.radius = 1.8;
	...
	const shadowMapAlgoType = {
		// EXPLAIN: depricated PCFSoftShadowMap, also is there
		// more available than we specified here
	...
	directionalShadowTweaks
		...
		.disable()
		.name(
			"radius (blur) doesn't work with `THREE.PCFSoftShadowMap`, but\nI also discovered PCFSoftShadowMap is deprecated",
		);
	directionalShadowTweaks
