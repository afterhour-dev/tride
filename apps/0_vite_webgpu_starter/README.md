...
- objasniti i sledeći navod:
   "Initialization is async. WebGPURenderer needs to negotiate a GPU adapter/device before it can render — this happens asynchronously under the hood. You have two options:
     - Call await renderer.init() once, then render normally with renderer.render(scene, camera).
     - Or skip init() and just call await renderer.renderAsync(scene, camera) each frame — it'll lazily init on first call." 
...
- u slučaju vite-a da li mi je potreban `vite-plugin-top-level-await`
